...
void rocksdb_backup_engine_create_new_backup_with_options_with_metadata(
  ...
  SaveError(errptr, be->rep->CreateNewBackupWithMetadata(bo->rep, db->rep, metadata_str, new_backup_id));
}
...
void rocksdb_create_backup_options_destroy(rocksdb_create_backup_options_t* opt) {
  delete opt;
}

void rocksdb_create_backup_options_set_flush_before_backup(rocksdb_create_backup_options_t* opt, bool flush) {
...
int* rocksdb_backup_engine_get_corrupted_backups(
	const rocksdb_backup_engine_t* be,
	size_t* num_corrupted_backups) {
	...
	*num_corrupted_backups = corrupt_backup_ids.size();
	int* corrupted_backups = static_cast<int*>(malloc(sizeof(int) * corrupt_backup_ids.size()));
...
void rocksdb_backup_engine_garbage_collect(rocksdb_backup_engine_t* be, char** errptr) {
...
void rocksdb_promote_l0(rocksdb_t* db, rocksdb_column_family_handle_t* column_family, int target_level, char** errptr) {
...
void rocksdb_writebatch_set_max_bytes(rocksdb_writebatch_t* b, size_t max_bytes) {
...
rocksdb_save_point_t* rocksdb_writebatch_get_wal_termination_point(rocksdb_writebatch_t* b) {
...
void rocksdb_save_point_clear(rocksdb_save_point_t* sp) {
  sp->rep.clear();
}
...
void rocksdb_save_point_destroy(rocksdb_save_point_t* sp) {
  delete sp;
}
...
void rocksdb_compactoptions_set_allow_write_stall(
    rocksdb_compactoptions_t* opt, unsigned char v) {
...
int rocksdb_compactoptions_get_max_subcompactions(rocksdb_compactoptions_t* opt) {
  return opt->rep.max_subcompactions;
}
...
void rocksdb_env_set_background_threads_with_priority(rocksdb_env_t* env, int n, int priority) {
  env->rep->SetBackgroundThreads(n, static_cast<Env::Priority>(priority));
}

int rocksdb_env_get_background_threads_with_priority(rocksdb_env_t* env, int priority) {
...
void rocksdb_env_lower_with_priority_thread_pool_io_priority(
...
void rocksdb_env_inc_background_threads_if_needed(rocksdb_env_t* env, int num, int priority) {
  env->rep->IncBackgroundThreadsIfNeeded(num, static_cast<Env::Priority>(priority));
}
