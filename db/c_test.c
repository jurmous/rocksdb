...
int main(int argc, char** argv) {
  ...
  StartPhase("backup_and_restore");
  {
    ...
    rocksdb_create_backup_options_t* beo = rocksdb_create_backup_options_create();
    rocksdb_create_backup_options_set_flush_before_backup(beo, false);

    uint32_t backup_id = 0;

    rocksdb_backup_engine_create_new_backup_with_options_with_metadata(be, db, beo, "a", &backup_id, &err);
    CheckNoError(err);
  ...
  StartPhase("writebatch_wi_vectors");
  {
    ...
    const char* v_list[3] = {"x", "y", "z"};
    const size_t v_sizes[3] = {1, 1, 1};
    rocksdb_writebatch_wi_putv(wb, 2, k_list, k_sizes, 3, v_list, v_sizes, &err);
    CheckNoError(err);
  ...
  StartPhase("env");
  {
    ...
    CheckCondition(15 == rocksdb_env_get_background_threads_with_priority(e, 1));
